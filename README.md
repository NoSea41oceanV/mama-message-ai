...
既定起動はキー不要のデモモードです。通常返答は、デモ保護者画像とMicrosoft Haruka音声を組み合わせた720x900の事前生成WebM動画です。保護者本人の声クローンやリアルタイム口・表情同期ではありません。OrcaRouterとOpenAI互換STTの実接続アダプターは実装済みですが、この環境にはAPIキーがないためライブ疎通と実測コストは未確認です。外部通信部分はfetch注入テストで検証しています。リアルタイム同期はMVPの合格条件に含めず、生成動画プロバイダーを安定して接続できた場合のみLEVEL 1として有効化します。自動テストは30件すべて成功しています。
