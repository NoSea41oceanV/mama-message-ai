...
- 本人同意済みデモ素材のサーバー検証
- 大人向け画面での保護者写真・10秒音声サンプル登録、顔・声ごとの明示同意、プレビュー、削除
- 登録写真と登録音声を使ったLEVEL 3返答（通常時のみ）と、未登録時のデモ動画フォールバック
...
既定起動はキー不要のデモモードです。大人向け画面から保護者の写真と共通フレーズの音声を登録すると、通常経路では本人写真・本人音声・同じ字幕をまとめたLEVEL 3返答を使います。素材はサーバーのメモリ内だけに保持し、削除操作またはサーバー再起動で消えます。未登録時は、デモ保護者画像とMicrosoft Haruka音声を組み合わせた720x900の事前生成WebM動画へ戻ります。保護者本人の声クローンやリアルタイム口・表情同期ではありません。OrcaRouterとOpenAI互換STTの実接続アダプターは実装済みですが、この環境にはAPIキーがないためライブ疎通と実測コストは未確認です。外部通信部分はfetch注入テストで検証しています。リアルタイム同期はMVPの合格条件に含めず、生成動画プロバイダーを安定して接続できた場合のみLEVEL 1として有効化します。
