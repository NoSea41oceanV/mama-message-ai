...
対象は、保育士、ベビーシッター、親族など、子どもを見守るすべての信頼できる大人です。MVPは同意管理を単純に保てる家庭内利用から始め、将来は認証と複数保護者の同意管理を整えて保育施設や一時預かりへ展開します。危険・深刻な相談では動画を生成せず、その場にいる信頼できる大人への相談を案内します。

起動時に「子どもと使う」「大人が確認する」を選べます。子ども側に表示される家庭IDを離れた保護者へ共有すると、大人側は同じ家庭IDと `.env` の `GUARDIAN_VIEW_ACCESS_CODE` で、会話をチャット形式で確認できます。会話本文は技術ログやGitへ保存せず、同じサーバーのメモリ上で既定30分だけ保持します。サーバー再起動時にも消去されます。本番運用では、家庭ID共有ではなく利用者認証、暗号化データベース、監査・削除機能へ置き換える前提です。

別端末から試す場合は、両端末から到達できる同じデプロイ先を使います。信頼できるLAN内で一時的に試す場合だけ `.env` を `HOST=0.0.0.0` にし、OSのファイアウォールと共有範囲を確認してください。インターネット公開ではHTTPSと正式なログイン認証を必須にします。
...
- 本人同意済みデモ素材のサーバー検証
- 大人向け画面での保護者写真・30秒〜2分の本人音声登録、顔・声・ElevenLabsへの外部送信ごとの明示同意、クローン音声プレビュー、プロフィール別の永続保存・削除
...
既定起動はキー不要のデモモードです。大人向け画面から保護者の写真と共通フレーズの音声を登録し、別途「返信動画をつくる」を実行すると、選択したD-IDまたはOrcaRouter経由のKlingが短い無音MP4を非同期生成します。D-IDではMicrosoft TTSのSSML break、`ja-JP-NanamiNeural`、`bank://lively/driver-06`、`fluent=true`を使い、音声を重複させずループ可能な約5秒の待機動画を作ります。生成には時間がかかるため会話ごとには待たず、完成動画を通常経路でループ再生し、発話ごとの返答文・動的音声・字幕を同時に重ねます。

本人声は、OpenAIの利用資格がある環境に限りCustom Voiceを任意で使用できます。ElevenLabs用に登録した音声をOpenAIへ自動送信してクローンするのではなく、OpenAI所定の同意録音から別途発行したVoice IDを使います。`.env`で`SPEECH_PROVIDER=openai-custom-voice`、`OPENAI_API_KEY`、`OPENAI_CUSTOM_VOICE_ID`、対応する`OPENAI_CUSTOM_VOICE_PROFILE_ID`をすべて設定した場合だけ有効です。同じVoice ID・モデル・返答文は30分間メモリキャッシュし、重複課金を避けます。音声は推測困難な短期URLだけで配信し、危険・不確実・同意無効の経路では外部音声APIを呼びません。資格・設定不足、失敗、タイムアウト時は既存のブラウザ読み上げへ戻るため、デモを停止しません。動画が未生成または失敗した場合は、登録写真の簡易アニメーションと動的読み上げを使うLEVEL 3へ戻ります。

OpenAI Custom Voiceを利用できない環境では、ElevenLabs Instant Voice Cloneも選択できます。`.env`へ`SPEECH_PROVIDER=elevenlabs`と`ELEVENLABS_API_KEY`を設定すると、保護者登録画面で30秒〜2分の録音、外部送信への明示同意、クローン作成、Voice IDの暗号化保存まで実行できます。既定モデルは日本語対応・低遅延・低価格の`eleven_flash_v2_5`です。既にVoice IDがある場合は`ELEVENLABS_VOICE_ID`と`ELEVENLABS_VOICE_PROFILE_ID`で固定する方法も残しています。OpenAIとElevenLabsは環境変数で切り替え、同時には呼びません。キャッシュ、安全ゲート、プロフィール固定、ブラウザ音声フォールバックは両方に適用されます。素材削除・再登録時にはElevenLabs側のクローン削除も試行します。

写真、声、生成MP4はブラウザが保持するランダムな保護者プロフィールIDごとに分離し、サーバー専用の `.data` へAES-256-GCMで暗号化保存します。プロバイダーの署名付き一時URLはブラウザへ返さず、サーバーがHTTPS・MIME・25MiB上限を検証して即時ダウンロードします。同じブラウザではサーバー再起動後も自動復元され、別プロフィールからは参照できません。素材の再登録・削除・同意停止では生成動画を含む暗号化データと素材URLを失効させます。未登録時は、デモ保護者画像とMicrosoft Haruka音声を組み合わせた720x900の事前生成WebM動画へ戻ります。Custom Voice有効時も、現行mainの動画は待機動画と別音声の同時再生であり、音素・顔ランドマークに基づく完全な口同期ではありません。
