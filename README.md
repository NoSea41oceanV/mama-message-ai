...
送信後は「おへんじをとどける準備をしているよ」と表示し、返答メディアと字幕がそろってから1つの返答として再生します。現在のアプリはOrcaRouterで`safetyLevel`、`supportMode`、`emotion`と返答文を構造化し、通常返答・保護者アバター返答・安全引き継ぎを監督します。保護者登録時にはD-IDまたはOrcaRouter経由のKlingへ写真を送り、生成済みMP4を暗号化保存できます。会話時はその動画をループし、発話ごとの動的音声と字幕を同時再生します。判定不能時は通常返信へ流さず同席大人へ引き継ぎます。
...
- 本人同意済みデモ素材のサーバー検証
- 大人向け画面での保護者写真・30秒音声サンプル登録、ElevenLabs Instant Voice Clone、顔・声ごとの明示同意、プレビュー、プロフィール別の永続保存・削除
- 登録写真からD-IDまたはKlingの実動画を非同期生成する大人向け操作、外部送信への明示同意、進捗・完成プレビュー
...
- OrcaRouter structured outputs、OpenAI互換STT、D-ID/Kling Video、LEVEL 1〜4メディアのプロバイダー境界
- 通常・生成失敗・安全・同意不備・冪等性・ログ秘匿を含む自動テスト
- CSP、Permissions Policy、入力長制限、危険・誤認表現の返答拒否

既定起動はキー不要のデモモードです。`VOICE_CLONING_PROVIDER=elevenlabs`を設定した実接続では、大人向け画面から保護者の写真と最大30秒の音声を登録すると、ElevenLabs Instant Voice Cloneの`voice_id`を作成します。会話時はOrcaRouterの返答文をElevenLabsの日本語TTSへ送り、生成音声・字幕と、D-IDまたはKlingで事前生成した短い待機動画を同時再生します。動画が未生成または失敗した場合も、登録写真の簡易アニメーションとクローン音声を使うLEVEL 3へ戻ります。ElevenLabsが一時的に失敗した場合だけブラウザ読み上げへフォールバックします。
...
実接続する場合は`.env.example`を参考に、Git管理対象外の`.env`へサーバー専用キーを設定します。会話判定は`ROUTER_PROVIDER=orcarouter`、動画生成は`VIDEO_GENERATION_PROVIDER=did`または`kling`、実STTは必要に応じて`STT_PROVIDER=openai`を指定してください。動画生成は大人向け画面で外部送信同意を確認して明示操作した場合だけ開始されます。`VIDEO_GENERATION_PROVIDER=disabled`へ戻すとキーが残っていても写真を外部送信しません。
...
保護者素材画面で30秒ほど自然に読み上げ、外部送信を含む声の利用同意を確認して登録します。登録完了後に「音声クローン済み」と表示されれば、次の通常返答からクローン音声が使われます。APIキーはサーバーだけが参照し、ブラウザやAPIレスポンスへ返しません。
