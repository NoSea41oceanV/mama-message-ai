...
既定起動はキー不要のデモモードです。大人向け画面から保護者の写真と共通フレーズの音声を登録し、別途「返信動画をつくる」を実行すると、選択したD-IDまたはOrcaRouter経由のKlingが短い無音MP4を非同期生成します。D-IDではMicrosoft TTSのSSML break、`ja-JP-NanamiNeural`、`bank://lively/driver-06`、`fluent=true`を使い、音声を重複させずループ可能な約5秒の待機動画を作ります。生成には時間がかかるため会話ごとには待たず、完成動画を通常経路でループ再生し、発話ごとの返答文・ブラウザ読み上げ・字幕を同時に重ねます。登録音声は本人確認用サンプルとして保存しますが、現在は任意の返答を本人声で合成する声クローンには使いません。動画が未生成または失敗した場合は、登録写真の簡易アニメーションと動的読み上げを使うLEVEL 3へ戻ります。
