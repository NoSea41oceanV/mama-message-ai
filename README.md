...
AI HACK 2026で開発した、保護者AIビデオメッセージです。

延長保育や一時預かりで保護者と長い時間会えず、寂しさや不安を感じている3〜6歳の未就学児へ、保護者が許可した声・写真などをもとに「その場にいない保護者からのAIビデオメッセージ」を届けるハッカソンプロジェクトです。保育士・ベビーシッターなど、子どものそばにいる信頼できる大人が一緒に使います。母親専用のプロダクトではなく、`Mama Movie` は主デモ上の例です。
...
主デモは、延長保育でお迎えを待つ子どもが、会えない寂しさを保育園の先生へ声で伝える場面です。同席する保育士やベビーシッターが操作を助け、届いた短いメッセージをきっかけに子どもをあやし、寂しさに寄り添います。「駄々をこねる子を黙らせる」用途でも、AIに育児を任せる用途でもありません。その場の大人による声かけや遊びへの誘導を補助します。

利用場面には、保育園の延長保育でお迎えを待つ時間、ベビーシッターによる一時預かり、保育施設で保護者と離れて過ごす時間を想定しています。保育士・シッター・一時預かりスタッフが操作を補助し、子どもだけで自由利用する設計にはしません。危険・深刻な相談では動画を生成せず、その場にいる信頼できる大人への相談を案内します。

## 離れた大人による会話確認

入口では子どもの「おはなしを はじめる」を主ボタンとして大きく表示し、大人用の導線は画面下部へ小さく分離しています。子ども側で発行される6桁のペアリングコードを離れた大人へ伝えると、大人はそのコードと `.env` の `GUARDIAN_VIEW_ACCESS_CODE` を使って、会話をチャット形式で確認できます。長い内部プロフィールIDを直接入力する必要はありません。

ペアリングコードは10分で失効し、コードだけでは会話を閲覧できません。会話本文は技術ログやGitへ保存せず、同じサーバーのメモリ上で `CONVERSATION_TTL_MINUTES` の期間だけ保持し、再起動時に消去します。本番運用では正式な利用者認証、暗号化データベース、試行回数制限、監査・削除機能へ置き換える前提です。
