...
既定起動はキー不要のデモモードです。LEVEL 3の音声はMicrosoft Harukaで事前生成したデモ音声であり、保護者本人の声クローンではありません。OrcaRouterとOpenAI互換STTの実接続アダプターは実装済みですが、この環境にはAPIキーがないためライブ疎通と実測コストは未確認です。外部通信部分はfetch注入テストで検証しています。リアルタイム口・表情同期はMVPの合格条件に含めず、生成動画プロバイダーを安定して接続できた場合のみLEVEL 1として有効化します。自動テストは29件すべて成功しています。
