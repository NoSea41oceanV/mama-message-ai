...
- 登録写真と登録音声を使ったLEVEL 3返答（音声連動の簡易表情アニメーション付き・通常時のみ）と、未登録時のデモ動画フォールバック
...
既定起動はキー不要のデモモードです。大人向け画面から保護者の写真と共通フレーズの音声を登録すると、通常経路では本人写真・本人音声・同じ字幕をまとめたLEVEL 3返答を使います。再生中は音声エネルギーに反応して写真の口元とうなずきを簡易アニメーションし、Web Audioが使えない環境ではCSSアニメーションへフォールバックします。素材はブラウザが保持するランダムな保護者プロフィールIDごとに分離し、サーバー専用の `.data` へAES-256-GCMで暗号化保存します。同じブラウザではサーバー再起動後も自動復元され、別プロフィールからは参照できません。削除・同意停止では暗号化ファイルと素材URLを失効させます。未登録時は、デモ保護者画像とMicrosoft Haruka音声を組み合わせた720x900の事前生成WebM動画へ戻ります。保護者本人の声クローンや、音素・顔ランドマークに基づく精密なリアルタイム口・表情同期ではありません。
