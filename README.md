...
主デモは、保護者が不在の場で信頼できる大人と過ごしている子どもが、嬉しかったことや寂しい気持ちを声で伝える場面です。同席する保育士、ベビーシッター、親族など、子どもを見守る大人が必要に応じて操作を助け、子どもに寄り添います。「駄々をこねる子を黙らせる」用途ではなく、その場の大人が寄り添う際の気持ちの切り替えを補助します。
...
入口では子どもの「おはなしを はじめる」を主ボタンとして大きく表示し、大人用の導線は画面下部へ小さく分離しています。子ども側で発行される6桁のペアリングコードを離れた大人へ伝えると、大人はそのコードと `.env` の `GUARDIAN_VIEW_ACCESS_CODE` を使って、会話をチャット形式で確認できます。長い家庭IDを直接入力する必要はありません。
