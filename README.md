...
送信後は「おへんじをとどける準備をしているよ」と表示し、返答メディアと字幕がそろってから1つの返答として再生します。現在のアプリはOrcaRouterで`safetyLevel`、`supportMode`、`emotion`と返答文を構造化し、通常返答・保護者アバター返答・安全引き継ぎを監督します。保護者登録時にはOrcaRouter経由でKlingへ写真を送り、生成済みMP4を暗号化保存できます。会話時はその動画をループし、発話ごとの動的音声と字幕を同時再生します。判定不能時は通常返信へ流さず同席大人へ引き継ぎます。
...
- 登録写真からKling実動画を非同期生成する大人向け操作、外部送信への明示同意、進捗・完成プレビュー
...
- `POST /api/sampling/video`、`GET /api/sampling/video/{jobId}`による非同期Kling生成
...
- OrcaRouter structured outputs、OpenAI互換STT、Kling Video、LEVEL 1〜4メディアのプロバイダー境界
- 通常・生成失敗・安全・同意不備・冪等性・ログ秘匿を含む自動テスト
- CSP、Permissions Policy、入力長制限、危険・誤認表現の返答拒否

既定起動はキー不要のデモモードです。大人向け画面から保護者の写真と共通フレーズの音声を登録し、別途「返信動画をつくる」を実行すると、OrcaRouter経由のKlingが約5秒の無音MP4を非同期生成します。生成には通常30〜90秒かかるため会話ごとには待たず、完成動画を通常経路でループ再生し、発話ごとの返答文・ブラウザ読み上げ・字幕を同時に重ねます。登録音声は本人確認用サンプルとして保存しますが、現在は任意の返答を本人声で合成する声クローンには使いません。動画が未生成または失敗した場合は、登録写真の簡易アニメーションと動的読み上げを使うLEVEL 3へ戻ります。

写真、声、生成MP4はブラウザが保持するランダムな保護者プロフィールIDごとに分離し、サーバー専用の `.data` へAES-256-GCMで暗号化保存します。Klingの署名付き一時URLはブラウザへ返さず、サーバーがHTTPS・MIME・25MiB上限を検証して即時ダウンロードします。同じブラウザではサーバー再起動後も自動復元され、別プロフィールからは参照できません。素材の再登録・削除・同意停止では生成動画を含む暗号化データと素材URLを失効させます。未登録時は、デモ保護者画像とMicrosoft Haruka音声を組み合わせた720x900の事前生成WebM動画へ戻ります。保護者本人の声クローンや、音素・顔ランドマークに基づく精密なリアルタイム口・表情同期ではありません。
...
実接続する場合は`.env.example`を参考に、Git管理対象外の`.env`へサーバー専用キーを設定します。会話判定は`ROUTER_PROVIDER=orcarouter`、動画生成は`VIDEO_GENERATION_PROVIDER=kling`、実STTは必要に応じて`STT_PROVIDER=openai`を指定してください。Kling生成は大人向け画面で外部送信同意を確認して明示操作した場合だけ開始されます。`VIDEO_GENERATION_PROVIDER=disabled`へ戻すとキーが残っていても写真を外部送信しません。既定値は`kling/kling-v3`の標準画像-to-動画、標準画質、5秒、音声なしです。動画の縦横比は登録写真から決まります。
...
2026-08-14時点のハッカソン環境では、APIキーが有効、上限`$20`、モデル制限なし、プロモーションクレジット`$19.98`（全モデル対象）であることを確認済みです。それでも`kling/kling-v3`は画像to動画と人物を含まないテキストto動画の両方が空本文の`HTTP 403`になり、OrcaRouterのリクエスト履歴にも記録されませんでした。この状態はアプリの画像形式やプロンプトではなく、OrcaRouter側のKling利用許可または課金経路の確認が必要です。解消後は素材画面の「返信動画をつくる」を押し直すだけで再生成できます。
