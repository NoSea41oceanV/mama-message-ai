...
本人声は、OpenAIの利用資格がある環境に限りCustom Voiceを任意で使用できます。登録した10秒音声を自動送信してクローンするのではなく、OpenAI所定の同意録音から別途発行したVoice IDを使います。`.env`で`SPEECH_PROVIDER=openai-custom-voice`、`OPENAI_API_KEY`、`OPENAI_CUSTOM_VOICE_ID`、対応する`OPENAI_CUSTOM_VOICE_PROFILE_ID`をすべて設定した場合だけ有効です。同じVoice ID・モデル・返答文は30分間メモリキャッシュし、重複課金を避けます。音声は推測困難な短期URLだけで配信し、危険・不確実・同意無効の経路では外部音声APIを呼びません。資格・設定不足、失敗、タイムアウト時は既存のブラウザ読み上げへ戻るため、デモを停止しません。動画が未生成または失敗した場合は、登録写真の簡易アニメーションと動的読み上げを使うLEVEL 3へ戻ります。
