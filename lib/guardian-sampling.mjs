...
const MAX_VOICE_SECONDS = 360;
const MAX_FAVORITE_TOPICS = 5;
const MAX_FAVORITE_TOPIC_LENGTH = 30;
const MAX_CHILD_NAME_LENGTH = 20;
const MAX_SELF_REFERENCE_LENGTH = 20;
const MAX_FAVORITE_ENDINGS = 5;
const MAX_FAVORITE_ENDING_LENGTH = 30;
const MAX_FAVORITE_PHRASES = 8;
const MAX_FAVORITE_PHRASE_LENGTH = 60;
const MAX_REPLY_EXAMPLES = 12;
const MAX_REPLY_EXAMPLE_SITUATION_LENGTH = 40;
const MAX_REPLY_EXAMPLE_TEXT_LENGTH = 120;
...
function normalizeTextList(value, { maximumItems, maximumLength }) {
  const source = Array.isArray(value) ? value : String(value ?? "").split(/[、,\n]/);
  const normalized = [];
  for (const item of source) {
    const text = String(item ?? "").replace(/[\r\n\t]+/g, " ").replace(/\s+/g, " ").trim();
    if (!text || text.length > maximumLength || normalized.includes(text)) continue;
...
export function normalizeSelfReference(value) {
  return String(value ?? "")
    .replace(/[\r\n\t]+/g, " ")
    .replace(/\s+/g, " ")
    .trim()
    .slice(0, MAX_SELF_REFERENCE_LENGTH);
}

export function normalizeFavoriteEndings(value) {
  return normalizeTextList(value, {
    maximumItems: MAX_FAVORITE_ENDINGS,
    maximumLength: MAX_FAVORITE_ENDING_LENGTH,
  });
}

export function normalizeFavoritePhrases(value) {
  return normalizeTextList(value, {
    maximumItems: MAX_FAVORITE_PHRASES,
    maximumLength: MAX_FAVORITE_PHRASE_LENGTH,
  });
}

export function normalizeReplyExamples(value) {
  const source = Array.isArray(value) ? value : [];
  const normalized = [];
  for (const item of source) {
    const situation = String(item?.situation ?? "").replace(/\s+/g, " ").trim().slice(0, MAX_REPLY_EXAMPLE_SITUATION_LENGTH);
    const reply = String(item?.reply ?? "").replace(/\s+/g, " ").trim().slice(0, MAX_REPLY_EXAMPLE_TEXT_LENGTH);
    if (!situation || !reply) continue;
    normalized.push({ situation, reply });
    if (normalized.length >= MAX_REPLY_EXAMPLES) break;
  }
  return normalized;
}

function speakingStyle(record) {
  return Object.freeze({
    selfReference: normalizeSelfReference(record?.selfReference ?? record?.subjectLabel),
    favoriteEndings: Object.freeze(normalizeFavoriteEndings(record?.favoriteEndings)),
    favoritePhrases: Object.freeze(normalizeFavoritePhrases(record?.favoritePhrases)),
    replyExamples: Object.freeze(normalizeReplyExamples(record?.replyExamples).map((item) => Object.freeze(item))),
  });
}
...
function publicStatus(record, state = record ? "READY" : "NOT_REGISTERED") {
  ...
  return Object.freeze({
    ...
    speakingStyle: speakingStyle(record),
...
export function createGuardianSamplingStore(options = {}) {
  ...
  let current = options.initialRecord ? {
    ...
    selfReference: normalizeSelfReference(options.initialRecord.selfReference ?? options.initialRecord.subjectLabel),
    favoriteEndings: normalizeFavoriteEndings(options.initialRecord.favoriteEndings),
    favoritePhrases: normalizeFavoritePhrases(options.initialRecord.favoritePhrases),
    replyExamples: normalizeReplyExamples(options.initialRecord.replyExamples),
  ...
  function register(input = {}, internal = {}) {
    ...
    try {
      ...
      const next = {
        ...
        selfReference: normalizeSelfReference(input.selfReference ?? input.subjectLabel),
        favoriteEndings: normalizeFavoriteEndings(input.favoriteEndings),
        favoritePhrases: normalizeFavoritePhrases(input.favoritePhrases),
        replyExamples: normalizeReplyExamples(input.replyExamples),
  ...
  function updatePreferences(input = {}) {
    ...
    current = {
      ...
      ...(Object.hasOwn(input, "speechRate")
        ? { speechRate: normalizeSpeechRate(input.speechRate) }
        : {}),
      ...(Object.hasOwn(input, "selfReference")
        ? { selfReference: normalizeSelfReference(input.selfReference) }
        : {}),
      ...(Object.hasOwn(input, "favoriteEndings")
        ? { favoriteEndings: normalizeFavoriteEndings(input.favoriteEndings) }
        : {}),
      ...(Object.hasOwn(input, "favoritePhrases")
        ? { favoritePhrases: normalizeFavoritePhrases(input.favoritePhrases) }
        : {}),
      ...(Object.hasOwn(input, "replyExamples")
        ? { replyExamples: normalizeReplyExamples(input.replyExamples) }
        : {}),
    };
    return publicStatus(current);
  ...
  function resolve(consentId, avatarAssetId) {
    ...
    return {
      ...
      speakingStyle: speakingStyle(current),
    };
  }
...
export const GUARDIAN_SAMPLING_LIMITS = Object.freeze({
  ...
  maxSelfReferenceLength: MAX_SELF_REFERENCE_LENGTH,
