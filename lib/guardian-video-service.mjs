...
export function createGuardianVideoService(options = {}) {
  ...
  const pollTimeoutMs = Math.max(
    pollIntervalMs,
    positiveNumber(provider.taskPollTimeoutMs, positiveNumber(options.pollTimeoutMs, 10 * 60 * 1000)),
  );
  const maximumBytes = positiveNumber(options.maximumBytes, 25 * 1024 * 1024);
    ...
    try {
      ...
      if (task.status === "ready") {
        if (task.prepared === true && !task.videoUrl) {
          // Some reusable photo avatars continue to depend on the uploaded
          // source asset. Providers that copy the image during training can
          // explicitly ask us to remove the short-lived source URL here.
          samplingStore.markVideoReady(profileId, jobId);
          if (task.cleanupProviderAsset === true) {
  ...
  function profileStatus(profileId) {
    let result = publicStatus(profileId);
    if (
      result?.status === "failed"
      && provider.recoverTimedOutTasks === true
      && typeof samplingStore.resumeTimedOutVideoJob === "function"
    ) {
      const record = internalRecord(profileId);
      const elapsed = now().getTime() - Date.parse(record?.createdAt ?? "");
      if (record?.providerTaskId && Number.isFinite(elapsed) && elapsed <= pollTimeoutMs) {
        result = samplingStore.resumeTimedOutVideoJob(profileId, record.jobId) ?? result;
        if (result.status === "processing") queueAdvance(profileId, record.jobId, 0);
      }
    }
    return result;
  }
  ...
  function profileProviderAssetId(profileId) {
    const status = publicStatus(profileId);
    if (status?.status !== "ready") return null;
    return internalRecord(profileId)?.providerAssetId ?? null;
  }
