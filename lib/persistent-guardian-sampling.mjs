...
export function createPersistentGuardianSamplingStore(options = {}) {
  ...
  return Object.freeze({
    ...
    updateVideoJob,
    storeVideo,
    storeVoiceClone: (profileId, clone) => {
      const normalized = assertProfileId(profileId);
      const store = load(normalized);
      if (clone?.provider !== "elevenlabs" || !/^[A-Za-z0-9_-]{8,}$/.test(String(clone.voiceId ?? ""))) return null;
      voiceClones.set(normalized, { provider: "elevenlabs", voiceId: String(clone.voiceId) });
      save(normalized, store);
      return { provider: "elevenlabs", configured: true };
    },
    voiceClone: (profileId) => voiceClones.get(assertProfileId(profileId)) ?? null,
    directory,
  });
}
