...
export function isLevel1BundleReady(bundle) {
  return bundle?.fallbackLevel === 1 && bundle.parentLike === true
    && nonEmptyString(bundle.videoUrl)
    && (bundle.audioInVideo === true || nonEmptyString(bundle.audioUrl) || bundle.speechSynthesis === true)
...
export function createMediaProvider(options = {}) {
  ...
  const preRecordedVideoUrl = options.preRecordedVideoUrl
    ...
    ?? null;
  const preRecordedVideoUrlForDecision = options.preRecordedVideoUrlForDecision ?? null;
  ...
  async function generate(input = {}) {
    ...
    const resolvedPosterUrl = nonEmptyString(input.posterUrl) ? input.posterUrl.trim() : posterUrl;
    const sampling = input.guardianSampling && typeof input.guardianSampling === "object"
      ? Object.freeze({ ...input.guardianSampling })
      : null;
    const context = { ...input, decision, replyText: subtitle, posterUrl: resolvedPosterUrl };
    ...
    if (generatedVideoProvider && !blocked(1)) {
      try {
        ...
        if (typeof generator === "function") {
          ...
          const audioInVideo = generated?.audioInVideo === true;
          const audioUrl = nonEmptyString(generated?.audioUrl) ? generated.audioUrl.trim() : null;
          const speechSynthesis = generated?.speechSynthesis === true;
          if (videoUrl && (audioInVideo || audioUrl || speechSynthesis)) {
            return readyBundle(MEDIA_LEVELS.GENERATED_VIDEO, {
              provider: generatedVideoProvider.name || providerName || "generated-video",
              videoUrl,
              posterUrl: generated?.posterUrl ?? resolvedPosterUrl,
              audioUrl,
              audioInVideo,
              speechSynthesis,
              browserSpeech: speechSynthesis,
              subtitle,
              syncLevel: audioInVideo || audioUrl ? "video-audio-subtitle" : "video-speech-subtitle",
              guardianSampling: sampling,
            });
          }
        }
      } catch {
    ...
    const resolvedPreRecordedVideoUrl = typeof preRecordedVideoUrlForDecision === "function"
      ? String(await preRecordedVideoUrlForDecision(decision, context) ?? "").trim()
      : String(preRecordedVideoUrl ?? "").trim();
    if (nonEmptyString(resolvedPreRecordedVideoUrl) && !blocked(2)) {
      return readyBundle(MEDIA_LEVELS.PRE_RECORDED_VIDEO, {
        provider: "pre-recorded-video",
        videoUrl: resolvedPreRecordedVideoUrl,
    ...
    if (nonEmptyString(resolvedPosterUrl) && !blocked(3)) {
      const candidateAudioUrl = Object.hasOwn(input, "audioUrl")
        ? nonEmptyString(input.audioUrl) ? input.audioUrl.trim() : ""
        : typeof audioUrlForDecision === "function"
          ? String(await audioUrlForDecision(decision, context) ?? "").trim()
          : "";
      const recordedAudioText = input.audioText ?? input.recordedAudioText ?? null;
      const audioTextKnown = nonEmptyString(recordedAudioText);
      const audioMatchesReply = Boolean(candidateAudioUrl) && (
        !audioTextKnown || comparableSpeechText(recordedAudioText) === comparableSpeechText(subtitle)
      );
      const audioUrl = audioMatchesReply ? candidateAudioUrl : "";
      const samplingMetadata = sampling ? {
        ...sampling,
        voiceSamplePurpose: sampling.voiceSamplePurpose ?? "sampling-confirmation",
        voiceSampleMatchesReply: Boolean(candidateAudioUrl) && audioTextKnown
          ? audioMatchesReply
          : sampling.voiceSampleMatchesReply ?? null,
        voiceUsed: sampling.voiceSampleRegistered === true ? Boolean(audioUrl) : sampling.voiceUsed === true,
        voiceCloningUsed: sampling.voiceCloningUsed === true && Boolean(audioUrl),
        voiceFallback: candidateAudioUrl && !audioMatchesReply ? "dynamic-tts" : sampling.voiceFallback ?? null,
      } : null;
      return readyBundle(MEDIA_LEVELS.STILL_SPEECH_SUBTITLE, {
        provider: audioUrl && sampling?.voiceCloningUsed
          ? sampling.voiceProvider ?? "custom-voice"
          : audioUrl ? "demo-generated-audio" : "browser-speech",
        posterUrl: resolvedPosterUrl,
        ...
        guardianSampling: samplingMetadata,
      });
    }
