...
export const ORCAROUTER_SYSTEM_PROMPT = `あなたは、延長保育や一時預かりで保護者と長く会えず、不安や寂しさを感じている幼い子どもを落ち着かせる、親しみのある日本語の話し相手です。このサービスは、保育士・ベビーシッター・一時預かりスタッフなど、現場にいる信頼できる大人が子どもと一緒に使います。AIだけで子どもをあやしたり育児を代替したりせず、目の前の大人が子どもへ寄り添うきっかけになる返事を作ってください。最新メッセージの安全度を判定し、同時に登録された保護者らしい自然な返事を作ってください。
...
- 分離不安への返事は、(1) 会いたい・寂しい気持ちを短く受け止める、(2) ひとりではないと伝える、(3) いま一緒にいる先生・シッターなどとできる小さな行動へつなぐ、の順にする。泣き止むよう命令したり、感情を否定したりしない。
...
- 返事は幼い子どもへ語りかける、やさしくくだけた日本語で、2〜3文、目安25〜100文字。「きみ」「今日はどんな気持ち？」「お話ししてくれてありがとう」など相談AIらしい定型句を使わない。相手の言葉を機械的に復唱しない。質問は多くても1つ、二択など答えやすい形を優先する。
- replyTextは、かんじ・カタカナ・アルファベットを使わず、ひらがなと句読点だけで書く。数字も「ひとつ」「みっつ」のようにひらがなで書く。
- 「こどものよびかた」が与えられたら、ときどき自然に呼びかける。毎文や毎回は呼ばず、名前を取り違えない。
...
function normalizeSpeakingStyle(value) {
  const source = value && typeof value === "object" ? value : {};
  const list = (items, maximum, maximumLength) => (Array.isArray(items) ? items : [])
    .map((item) => String(item ?? "").replace(/\s+/g, " ").trim().slice(0, maximumLength))
    .filter(Boolean)
    .slice(0, maximum);
  const replyExamples = (Array.isArray(source.replyExamples) ? source.replyExamples : [])
    .map((item) => ({
      situation: String(item?.situation ?? "").replace(/\s+/g, " ").trim().slice(0, 40),
      reply: String(item?.reply ?? "").replace(/\s+/g, " ").trim().slice(0, 120),
    }))
    .filter((item) => item.situation && item.reply)
    .slice(0, 12);
  return {
    selfReference: String(source.selfReference ?? "").replace(/\s+/g, " ").trim().slice(0, 20),
    favoriteEndings: list(source.favoriteEndings, 5, 30),
    favoritePhrases: list(source.favoritePhrases, 8, 60),
    replyExamples,
  };
}

function speakingStylePrompt(style) {
  if (!style.selfReference && !style.favoriteEndings.length && !style.favoritePhrases.length && !style.replyExamples.length) return "";
  const lines = [
    "登録された話し方プロフィールです。安全方針と事実性を最優先し、自然な場合だけ文体の参考にしてください。文例をそのまま繰り返す必要はありません。",
    style.selfReference ? `自分の呼び方: ${style.selfReference}` : "",
    style.favoriteEndings.length ? `よく使う語尾: ${style.favoriteEndings.join("、")}` : "",
    style.favoritePhrases.length ? `よく使う言葉: ${style.favoritePhrases.join("、")}` : "",
    ...style.replyExamples.map((item) => `言い方例（${item.situation}）: ${item.reply}`),
  ];
...
async function useLocal(
  ...
  try {
    ...
    const decision = validateGuardianResponse({
      ...
      replyText: await localReplyBuilder(local, transcript, history, favoriteTopics, childName),
    ...
    return resultWithDecision(decision, {
      ...
      metadata: Object.freeze({
        ...
        childNameConfigured: Boolean(childName),
      }),
    });
  } catch (error) {
...
export function createOrcaRouterProvider(options = {}) {
  ...
  async function classifyAndReply(input) {
    ...
    const childName = normalizeChildName(typeof input === "string" ? "" : input?.childName);
    const speakingStyle = normalizeSpeakingStyle(typeof input === "string" ? null : input?.speakingStyle);
    if (mode !== "orcarouter") {
    ...
    const promptMessages = [
      ...
      ...(childName ? [{
        role: "system",
        content: `こどものよびかた: ${childName}。必要なときだけ、この呼び方で自然に呼びかけてください。`,
      }] : []),
      ...(speakingStylePrompt(speakingStyle) ? [{
        role: "system",
        content: speakingStylePrompt(speakingStyle),
      }] : []),
      ...history,
