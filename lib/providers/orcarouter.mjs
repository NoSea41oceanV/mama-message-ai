...
async function useLocal(transcript, localClassifier, localReplyBuilder) {
  try {
    const local = await localClassifier(transcript);
    const presentation = {
    ...
    const decision = validateGuardianResponse({
      ...
      replyText: await localReplyBuilder(local, transcript),
    ...
    return resultWithDecision(decision, {
      ...
      ok: true,
      error: Object.freeze({
        code: "ORCAROUTER_API_KEY_MISSING",
        message: "OrcaRouter is unavailable; injected local fallback was used",
      }),
      metadata: Object.freeze({ provider: "local", fallback: true }),
    });
  } catch (error) {
...
function unavailable(model) {
  return Object.freeze({
    ...
    error: Object.freeze({
      code: "ORCAROUTER_API_KEY_MISSING",
      message: "ORCAROUTER_API_KEY is not configured",
    }),
...
export function createOrcaRouterProvider(options = {}) {
  ...
  async function classifyAndReply(input) {
    ...
    if (!apiKey && typeof localClassifier === "function" && typeof localReplyBuilder === "function") {
      return useLocal(transcript, localClassifier, localReplyBuilder);
    }
      ...
      response = await fetchImpl(`${baseUrl}/chat/completions`, {
        ...
        body: JSON.stringify({
          ...
          messages: [
            {
              role: "system",
              content: "Classify this young child's Japanese message and draft a brief supportive reply. Any ambiguity or safety concern must use adult_handoff. Return only the required structured fields.",
            },
