...
export function createTavusProvider(options = {}) {
  ...
  const taskPollTimeoutMs = boundedSeconds(
    options.taskPollTimeoutSeconds ?? env.TAVUS_FACE_POLL_TIMEOUT_SECONDS,
    5 * 60 * 60,
    10 * 60,
    24 * 60 * 60,
  ) * 1000;
  ...
  async function deleteTask(taskId) {
    const id = encodeURIComponent(String(taskId ?? "").trim());
    if (!id) return false;
    await request(`/v2/faces/${id}?hard=true`, { method: "DELETE" });
    return true;
  }
